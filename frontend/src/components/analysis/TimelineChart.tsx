...
export default function TimelineChart({ posts, dateCol = 'date', scoreCol = 'risk_score' }: TimelineChartProps) {
  // Safety check: If Plotly failed to load, don't crash the app
  if (!Plot) {
    return <div className="p-4 text-sm text-gray-500">Chart engine failed to load. Please refresh.</div>
  }

  try {
    const parsed = posts
      .map((p) => {
        const d = new Date(p[dateCol as keyof PostData] as string)
        return { ...p, parsedDate: d }
      })
    ...
    if (parsed.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center h-[200px] text-[#c4c9d0] gap-[5px]">
          ...
          <span className="text-[0.65rem]">No valid dates found in posts to display timeline</span>
        </div>
      )
    }
    ...
    console.error('TimelineChart render error:', err)
    return <div className="p-4 text-xs text-red-500 bg-red-50 rounded">Could not render timeline chart.</div>
  }
}
